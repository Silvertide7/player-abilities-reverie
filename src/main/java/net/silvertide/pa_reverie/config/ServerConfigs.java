...
public final class ServerConfigs {
    ...
    static {
        ...
        BUILDER.comment("Health restored per point of the eaten food's nutrition. Final heal = nutrition x this x level potency x quality bonus, then scaled by harvest spell power.");
        FEAST_OF_LIFE_HEALTH_PER_NUTRITION = BUILDER.defineInRange("feastOfLifeHealthPerNutrition", 1.0, 0.0, 100.0);

        BUILDER.comment("Absorption granted per point of the eaten food's saturation modifier. Final absorption = saturation x this x level potency x quality bonus, then scaled by harvest spell power.");
